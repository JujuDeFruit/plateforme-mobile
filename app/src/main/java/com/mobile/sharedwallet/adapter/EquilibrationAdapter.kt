...
class EquilibrationAdapter(private val dataSet : ArrayList<ArrayList<String>> = ArrayList()) : BaseAdapter() {
    ...
    override fun getView(p0: Int, p1: View?, parent: ViewGroup?): View {
        ...
        view.findViewById<ImageView>(R.id.equilibrationRowDebterPhoto).setImageBitmap(participant1.photo)

        val participant2 : Participant = Shared.pot.participants.first { it.uid == item[1] }
        view.findViewById<ImageView>(R.id.equilibrationRowReceiverPhoto).setImageBitmap(participant2.photo)
